...
const Header = ({ setIsSidebar }) => {
    ...
    return (
        <header className='flex px-3 lg:px-7 bg-white'>
            <div className='flex justify-between items-center w-full  border-b boroder-[#F0F0F0] py-[13.5px]'>
                <div className='flex items-center gap-x-3'>
                    <div className='lg:hidden'>
                        <HiMenuAlt1 onClick={() => setIsSidebar((prev) => !prev)} className="text-main text-2xl" />
                    </div>
                    <h3 className='text-primary font-semibold text-lg xs:text-2xl sm:text-[26px] capitalize'>{headings.find((route) => pathname.includes(route.path) || pathname.startsWith(route.path))?.heading ?? "Not Found"}</h3>
                </div>
